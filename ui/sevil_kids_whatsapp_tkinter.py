...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, current_phone):
        ...
        tk.Label(
            self,
            text="Этот номер будет подставляться в тестовый диалог.",
            bg="#111b21",
            fg="#8696a0",
            font=("Segoe UI", 9),
...
class WhatsAppClone(tk.Tk):
    def __init__(self):
        ...
        self.chats = [
            {"name": "Sevil Kids", "subtitle": "Бот клиники · готов к тестам", "time": "20:23", "unread": 0, "color": "#53bdeb"},
        ...
        self.messages = {
            "Sevil Kids": [
                {"from_me": False, "text": "Здравствуйте! Это тестовый бот Sevil Kids.\nЯ могу имитировать ответы и позже дергать ваши DigitalOcean Functions.", "time": "18:00"},
                {"from_me": False, "text": f"Текущий тестовый номер клиента: {self.user_phone}", "time": "18:00"},
            ],
            "CRM-Sevil kids": [{"from_me": False, "text": "Напомни завтра проверить incoming-message и create visit.", "time": "19:42"}],
            "Жаным❤️": [{"from_me": False, "text": "Спишь", "time": "19:43"}],
            "Гульчитай Мырзагали": [{"from_me": False, "text": "Ок", "time": "19:41"}],
            "Жансая Sevil": [{"from_me": False, "text": "Завтра в 14:00 вам удобно будет?", "time": "19:17"}],
            "Туракбаевтар💰❤️": [{"from_me": False, "text": "Рқоша: 🎤 0:35", "time": "18:33"}],
            "Нуртас 2GIS": [{"from_me": False, "text": "Хорошо, завтра созвонимся", "time": "18:01"}],
            "Elite Force League": [{"from_me": False, "text": "Регламент уже финальный?", "time": "17:48"}],
            "Casa Interiors": [{"from_me": False, "text": "Нужен еще один рендер", "time": "17:15"}],
            "Алина": [{"from_me": False, "text": "Нурали уже дома", "time": "16:22"}],
        }
    ...
    def build_sidebar(self):
        ...
        tk.Label(install, text="🟢  Установите WhatsApp для Windows", bg="#25d366", fg="#111b21", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=12, pady=8)
    ...
    def build_chat_area(self):
        ...
        self.chat_subtitle = tk.Label(title_wrap, text="Бот клиники · готов к тестам", bg="#202c33", fg="#8696a0", font=("Segoe UI", 10))
    ...
    def update_header(self):
        ...
        self.chat_subtitle.config(text=chat["subtitle"] if chat["name"] != "Sevil Kids" else "Бот клиники · готов к тестам")
    ...
    def render_messages(self):
        ...
        spacer = tk.Frame(self.msg_frame, bg="#0b141a", height=18)
        spacer.pack(fill="x")
    ...
    def send_message(self):
        ...
        self.messages.setdefault(self.selected_chat, []).append({"from_me": True, "text": text, "time": now})
        self.message_var.set("")
        self.render_messages()

        if self.selected_chat == "Sevil Kids":
            self.after(450, lambda: self.bot_reply(text))

    def bot_reply(self, text):
        t = text.lower()
        if "привет" in t or "здрав" in t:
            reply = "Здравствуйте! Я тестовый бот Sevil Kids.\nСейчас это визуальная оболочка под будущие тесты."
        elif "номер" in t:
            reply = f"Текущий тестовый номер клиента: {self.user_phone}\nИзменить его можно через настройки слева снизу."
        elif "запис" in t:
            reply = "Отлично. Здесь позже подключим сценарий записи к врачу через ваши функции."
        elif "пациент" in t or "клиент" in t:
            reply = "Здесь будет проверка пациента в базе по номеру телефона."
        else:
            reply = "Сообщение получено."

        self.messages["Sevil Kids"].append({"from_me": False, "text": reply, "time": datetime.now().strftime("%H:%M")})
        self.render_messages()
    ...
    def open_settings(self):
        ...
        if dialog.result:
            ...
            if len(self.messages["Sevil Kids"]) >= 2:
                self.messages["Sevil Kids"][1]["text"] = f"Текущий тестовый номер клиента: {self.user_phone}"
            self.render_messages()
