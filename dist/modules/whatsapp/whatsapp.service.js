...
let WhatsAppService = WhatsAppService_1 = class WhatsAppService {
    constructor(dentistService, bitrixService, bitrixOpenLinesService, sessionService, messages, bookingDraftService, scriptCatalog, geminiNluService) {
    ...
    async handleIncoming(input) {
        ...
        try {
            ...
            if (this.requestsHuman(lower)) {
                this.logger.log(`Handoff requested by client for ${session.normalizedPhone}; switching mode to WAITING_OPERATOR`);
            ...
            if (session.currentMode !== 'AUTO') {
                this.logger.log(`Reply suppressed for ${session.normalizedPhone}: currentMode=${session.currentMode}, allowBotReplies=${session.allowBotReplies}, botEnabled=${session.botEnabled}`);
                return {
                    reply: null,
                    session,
                    suppressReply: true,
            ...
            if (session.awaitingChildData) {
                ...
                return {
                    reply: this.messages.t(session.language, 'Спасибо. Теперь подскажите удобную дату или время для записи.', 'Рақмет. Енді өзіңізге ыңғайлы күнді немесе уақытты жаза аласыз ба?'),
                    session,
                };
            }
...
Врач: ${selected.doctorName}`,
    ...
    getBookingCompletionReply(session, booked) {
        if (booked.visitId === 0) {
            if (this.serviceRequiresPrepayment(session.selectedService)) {
                return this.scriptCatalog.getConsultationPrepayment();
            }
            return this.messages.t(session.language, 'Я передала запись оператору. Он продолжит оформление и свяжется с вами без потери контекста.', 'Жазбаны операторға бердім. Ол әңгіменің контекстін жоғалтпай жалғастырады.');
        }
    ...
    serviceRequiresPrepayment(service) {
    ...
    async moveSessionToHuman(session, input) {
