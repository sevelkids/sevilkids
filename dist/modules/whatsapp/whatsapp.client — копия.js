...
function t(lang, ru, kk) {
    return lang === 'kk' ? kk : ru;
}
function getMainGreeting(lang, name) {
    if (name) {
        return t(lang, `${name}, здравствуйте! Я бот Жансая клиники Sevil Kids. Помогу вам с записью на приём. Сейчас у нас доступны консультация и чистка. Что вас интересует?`, `${name}, сәлеметсіз бе! Мен Sevil Kids клиникасының Жансая ботымын. Қабылдауға жазылуға көмектесемін. Қазіргі таңда бізде консультация және тіс тазалау қызметтері бар. Сізді қайсысы қызықтырады?`);
    }
    return t(lang, 'Здравствуйте! Я бот Жансая клиники Sevil Kids. С радостью помогу вам с записью на приём. Сейчас у нас доступны консультация и чистка. Что вас интересует?', 'Сәлеметсіз бе! Мен Sevil Kids клиникасының Жансая ботымын. Қабылдауға жазылуға көмектесемін. Қазіргі таңда бізде консультация және тіс тазалау қызметтері бар. Сізді қайсысы қызықтырады?');
}
function getNotFoundAskName(lang) {
    return t(lang, 'Здравствуйте! Я бот Жансая клиники Sevil Kids. Не смогла найти вас в базе. Подскажите, пожалуйста, ваше имя, и я продолжу оформление записи.', 'Сәлеметсіз бе! Мен Sevil Kids клиникасының Жансая ботымын. Сізді базадан таба алмадым. Жазылуды жалғастыру үшін атыңызды жаза аласыз ба?');
}
function getAskNameAgain(lang) {
    return t(lang, 'Подскажите, пожалуйста, ваше имя, чтобы я продолжила оформление записи.', 'Жазылуды жалғастыру үшін атыңызды жаза аласыз ба?');
}
function getServicesText(lang) {
    return t(lang, 'Сейчас у нас доступны консультация и чистка. Что вас интересует?', 'Қазіргі таңда бізде консультация және тіс тазалау қызметтері бар. Сізді қайсысы қызықтырады?');
}
function getConsultationAgePrompt(lang) {
    return t(lang, 'Подскажите, пожалуйста, возраст пациента, чтобы я точно подсказала стоимость консультации.', 'Консультация бағасын нақты айту үшін пациенттің жасын жаза аласыз ба?');
}
function getConsultationPrice(lang, age) {
    if (age < 16) {
        return t(lang, 'Стоимость консультации до 16 лет — 8 000 тг.', '16 жасқа дейінгі консультация құны — 8 000 тг.');
    }
    return t(lang, 'Стоимость консультации от 16 лет — 10 000 тг.', '16 жастан бастап консультация құны — 10 000 тг.');
}
function getCleaningPriceList(lang) {
    return t(lang, [
...
function getCleaningTypePrompt(lang) {
    return t(lang, [
...
function getPostRegistrationPrompt(lang, firstName) {
    return t(lang, `${firstName}, отлично, я зарегистрировала вас в системе. Сейчас у нас доступны консультация и чистка. Что вас интересует?`, `${firstName}, керемет, мен сізді жүйеге тіркедім. Қазіргі таңда бізде консультация және тіс тазалау қызметтері бар. Сізді қайсысы қызықтырады?`);
}
function getAskDateTimeText(lang, serviceLabel) {
    return t(lang, `Хорошо, записываем на ${serviceLabel}. Подскажите, пожалуйста, удобную дату или время.`, `Жақсы, сізді ${serviceLabel} қызметіне жазамыз. Өзіңізге ыңғайлы күнді немесе уақытты жаза аласыз ба?`);
}
function getDateTimeAccepted(lang, serviceLabel, datetimeText) {
    return t(lang, `Приняла. Вы хотите записаться на ${serviceLabel} — ${datetimeText}. Сейчас следующим шагом я проверю доступные свободные окна и врачей.`, `Түсіндім. Сіз ${serviceLabel} қызметіне ${datetimeText} уақытына жазылғыңыз келеді. Келесі қадамда мен бос уақыттарды және қолжетімді дәрігерлерді тексеремін.`);
}
function detectCleaningTypeFromText(text) {
...
function getCleaningTypePrice(type) {
    const prices = {
...
async function askGeminiForIntent(text) {
    const prompt = `
...
async function createPatient(phoneNumber, firstName) {
    const normalizedPhone = normalizePhone(phoneNumber);
    ...
    const patientName = data.fullName || [data.lastName, data.firstName].filter(Boolean).join(' ').trim() || firstName;
...
client.on('message', async (message) => {
    try {
        ...
        if (session.awaitingConsultationAge) {
            ...
            await message.reply(`${getConsultationPrice(session.language, age)} ${t(session.language, 'Если хотите, я сразу помогу записаться на консультацию.', 'Қаласаңыз, мен бірден консультацияға жазылуға көмектесемін.')}`);
            return;
        }
        let parsed = {
        ...
        if (parsed.intent === 'ask_price' && parsed.service === 'consultation') {
            ...
            await message.reply(`${getConsultationPrice(session.language, parsed.age)} ${t(session.language, 'Если хотите, я сразу помогу записаться на консультацию.', 'Қаласаңыз, мен бірден консультацияға жазылуға көмектесемін.')}`);
            return;
        }
        ...
        if ((parsed.intent === 'choose_service' || parsed.intent === 'booking_request') &&
        ...
        if ((parsed.intent === 'choose_service' || parsed.intent === 'booking_request') &&
            ...
            if (parsed.cleaning_type) {
                ...
                session.awaitingDateTime = true;
                await message.reply(`${t(session.language, `Хорошо, выбираем ${getCleaningTypeLabel(session.language, parsed.cleaning_type)}. Стоимость — ${session.selectedPrice} тг.`, `${getCleaningTypeLabel(session.language, parsed.cleaning_type)} таңдалды. Құны — ${session.selectedPrice} тг.`)} ${t(session.language, 'Подскажите, пожалуйста, удобную дату или время.', 'Өзіңізге ыңғайлы күнді немесе уақытты жаза аласыз ба?')}`);
                return;
            }
        ...
        if (session.awaitingCleaningType) {
            ...
            session.awaitingDateTime = true;
            await message.reply(`${t(session.language, `Хорошо, выбрана ${getCleaningTypeLabel(session.language, type)}. Стоимость — ${session.selectedPrice} тг.`, `${getCleaningTypeLabel(session.language, type)} таңдалды. Құны — ${session.selectedPrice} тг.`)} ${t(session.language, 'Подскажите, пожалуйста, удобную дату или время.', 'Өзіңізге ыңғайлы күнді немесе уақытты жаза аласыз ба?')}`);
            return;
        }
        if (parsed.intent === 'provide_datetime' || session.awaitingDateTime) {
            ...
            if (!parsed.datetime_text) {
                await message.reply(t(session.language, 'Подскажите, пожалуйста, удобную дату или время, чтобы я проверила свободные окна.', 'Бос уақыттарды тексеру үшін ыңғайлы күнді немесе уақытты жаза аласыз ба?'));
                return;
            }
            session.selectedDateTimeText = parsed.datetime_text;
            session.awaitingDateTime = false;
            session.awaitingBookingConfirmation = true;
            const serviceLabel = session.selectedService === 'consultation'
        ...
        await message.reply(t(session.language, 'Я помогу вам с записью. Сейчас у нас доступны консультация и чистка. Напишите, пожалуйста, что вас интересует.', 'Мен сізге жазылуға көмектесемін. Қазіргі таңда бізде консультация және тіс тазалау қызметтері бар. Сізді не қызықтыратынын жаза аласыз ба?'));
    }
    catch (error) {
        console.error('Message handler error:', error);
        await message.reply('Извините, произошла ошибка при обработке сообщения. Попробуйте, пожалуйста, ещё раз чуть позже.');
    }
});
client.initialize();
//# sourceMappingURL=whatsapp.client%20%E2%80%94%20%D0%BA%D0%BE%D0%BF%D0%B8%D1%8F.js.map