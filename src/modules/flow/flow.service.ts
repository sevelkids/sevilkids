...
export class FlowService {
  ...
  private normalizeStatusForKey(status: unknown): string {
    if (!status) {
      return 'unknown';
    }
  ...
  private commentAlreadyExists(existingComments?: string | null, comment?: string | null): boolean {
  ...
  private async appendCommentIfChanged(
      dealId: number | null,
  ...
  private async updateStageIfNeeded(
      dealId: number | null,
  ...
  private async resolveBitrixState(patientId: number, visitId: number) {
    const contact = await this.bitrixService.findContactByDentistPlusPatientId(patientId);

    let requestDeal: any = null;

    if (contact?.ID) {
      requestDeal = await this.bitrixService.findOpenRequestDealByContactId(
          Number(contact.ID),
      );
    }
  ...
  async processSendVisitReminder(input: {
    ...
    return {
      ...
      sent: false,
      reason:
          'Текст напоминания сформирован. Реальная отправка будет подключена через WhatsApp-провайдера.',
      patientId: input.patientId,
      visitId: input.visitId,
  ...
  async processSyncKnownVisit(input: {
    ...
    if (!visitDeal?.ID) {
      return {
        ...
        skip: true,
        reason:
            'В Bitrix нет визитной сделки с таким Dentist Plus visitId. Чужой визит не импортируем.',
        visitId: input.visitId,
      };
    }

    let visit: Awaited<ReturnType<DentistService['getVisit']>> | null = null;
    ...
    if (missingInDentist || !visit) {
      if (!input.patientId) {
        throw new Error(
            'Sync failed: patientId is required when visit is missing in Dentist Plus',
        );
      }
      ...
      const requestStageChanged = await this.updateStageIfNeeded(
          state.requestDealId,
          state.requestDeal?.STAGE_ID,
          this.bitrixService.getStageIds().requests.stages.THINKING,
      );

      const visitStageChanged = await this.updateStageIfNeeded(
          state.visitDealId,
          state.visitDeal?.STAGE_ID,
          this.bitrixService.getStageIds().visits.stages.LOSE,
      );

      const requestCommentAdded = await this.appendCommentIfChanged(
          state.requestDealId,
          state.requestDeal?.COMMENTS,
          comment,
      );

      const visitCommentAdded = await this.appendCommentIfChanged(
          state.visitDealId,
          state.visitDeal?.COMMENTS,
          comment,
      );
    ...
    if (cancelled) {
      visitStageChanged = await this.updateStageIfNeeded(
          state.visitDealId,
          state.visitDeal?.STAGE_ID,
          this.bitrixService.getStageIds().visits.stages.LOSE,
      );
