...
import { ParsedIntent, Language } from '../whatsapp/whatsapp.types';
...
export class GeminiNluService {
    ...
    async parseIntent(text: string, currentLanguage: Language = 'ru'): Promise<ParsedIntent> {
    ...
    private buildPrompt(text: string, currentLanguage: Language): string {
        return `
...
`.trim();
    ...
    private fallbackIntent(text: string, currentLanguage: Language): ParsedIntent {
        const lower = text.toLowerCase();

        const consultation =
            lower.includes('консульта') ||
        ...
        let service: ParsedIntent['service'] = null;

        if (greeting) intent = 'greeting';
        if (asksServices) intent = 'ask_services';
        ...
        const language: Language =
            /[әіңғүұқөһ]/i.test(text) || lower.includes('сәлем') || lower.includes('қандай')
                ? 'kk'
                : currentLanguage || 'ru';
    ...
    private normalizeIntent(value: any): ParsedIntent['intent'] {
        const allowed: ParsedIntent['intent'][] = [
            'greeting',
            'ask_services',
            'ask_price',
            'choose_service',
    ...
    private normalizeService(value: any): ParsedIntent['service'] {
    ...
    private normalizeCleaningType(value: any): ParsedIntent['cleaning_type'] {
        const allowed: NonNullable<ParsedIntent['cleaning_type']>[] = [
            'light_milk_bite',
            'medium_milk_bite',
            'airflow_glycine_upto9',
            'airflow_glycine_10_16',
    ...
    private normalizeClarification(value: any): ParsedIntent['clarification_for'] {
