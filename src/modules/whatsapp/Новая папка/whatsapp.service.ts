...
export class WhatsAppService {
    ...
    async handleIncoming(input: WhatsAppInboundMessage): Promise<HandleIncomingResult> {
        ...
        // 1. Если ждем имя — создаем пациента, потом новый лид
        ...
        // 3. Сначала всегда пациент
        await this.ensurePatient(input, session);

        // 4. Если есть активная запись — сначала говорить про нее
        ...
        // 5. Если нет активной записи — создаем новый lead обращения, если еще нет
        await this.ensureLead(input, session);

        // 6. Если ждем возраст консультации
        ...
        if (intent.intent === 'provide_datetime' || session.awaitingDateTime) {
            ...
            // Только дата -> сначала даем время на выбор
