...
import { ChatSession } from '../../whatsapp/whatsapp.types';

type InstallCallbackContext = { contentType?: string | null; method?: string | null };

type ExtractedInstallAuth = {
...
type StoredInstallAuth = ExtractedInstallAuth & {
    id: string;
    connectorId: string | null;
    lineId: string | null;
    connectorRegisteredAt: string | null;
    connectorRegistrationStatus: string | null;
    connectorActivatedAt: string | null;
    connectorActivationStatus: string | null;
    lastError: string | null;
    installedAt: string;
    updatedAt: string;
};

type ConnectorAttemptResult = {
    attempted: boolean;
    success: boolean;
    skipped: boolean;
    reason?: string;
...
@Injectable()
export class BitrixOpenLinesService {
    private readonly logger = new Logger(BitrixOpenLinesService.name);
    private readonly enabled = String(process.env.BITRIX_OPENLINE_ENABLED || 'false').toLowerCase() === 'true';
    private readonly connectorId = process.env.BITRIX_OPENLINE_CONNECTOR_ID || '';
    private readonly lineId = process.env.BITRIX_OPENLINE_LINE_ID || '';
    private readonly appMode = process.env.BITRIX_OPENLINE_APP_MODE || 'webhook';
    private readonly publicBaseUrl = (process.env.BITRIX_OPENLINE_PUBLIC_BASE_URL || '').replace(/\/+$/, '');
    private readonly connectorName = process.env.BITRIX_OPENLINE_CONNECTOR_NAME || 'Sevil Kids WhatsApp';
    ...
    async handleInstallCallback(payload: Record<string, unknown>, context: InstallCallbackContext = {}) {
        ...
        let persistedAuth: StoredInstallAuth | null = null;
    ...
    async handleAppEvent(payload: Record<string, unknown>) {
        const normalizedPayload = this.normalizePayload(payload);
    ...
    async handleDeliveryCallback(payload: Record<string, unknown>) {
        const normalizedPayload = this.normalizePayload(payload);
    ...
    async forwardClientMessage(
        session: ChatSession,
        input: { text: string; phoneNumber: string; whatsappMessageId?: string | null; payload?: Record<string, unknown> },
    ): Promise<{ forwarded: boolean; reason?: string }> {
        if (!this.enabled) return { forwarded: false, reason: 'BITRIX_OPENLINE_ENABLED=false' };
        ...
        if (session.leadId) {
            try {
                await this.bitrixService.appendDealComment(
                    session.leadId,
                    `Open Lines mirror: incoming client message${input.whatsappMessageId ? ` (${input.whatsappMessageId})` : ''}\n${input.text}`,
                );
            } catch (error: any) {
    ...
    async handleOperatorEvent(
        session: ChatSession,
        input: { eventType: string; operatorId?: string | null; lineId?: string | null; chatId?: string | null; dialogId?: string | null; payload?: Record<string, unknown> },
    ) {
        if (input.lineId) session.bitrixLineId = input.lineId;
        if (input.chatId) session.bitrixChatId = input.chatId;
        if (input.dialogId) session.bitrixDialogId = input.dialogId;
        if (this.connectorId) session.bitrixConnectorId = this.connectorId;
        if (input.operatorId) session.assignedOperatorId = input.operatorId;
        this.logger.log(`Received Bitrix operator event ${input.eventType} for ${session.normalizedPhone}`);
        if (session.leadId) {
            try {
                await this.bitrixService.appendDealComment(
                    session.leadId,
                    `Bitrix operator event: ${input.eventType}${input.operatorId ? `, operator=${input.operatorId}` : ''}`,
                );
            } catch (error: any) {
    ...
    async registerConnector(authOverride?: StoredInstallAuth | null): Promise<ConnectorAttemptResult> {
    ...
    async activateConnector(authOverride?: StoredInstallAuth | null): Promise<ConnectorAttemptResult> {
        const auth = authOverride || (await this.getLatestInstallAuth());
        if (!auth) {
            const reason = 'Install auth payload is not persisted yet';
    ...
    async ensureConnectorReadyAfterInstall(auth: StoredInstallAuth) {
        const registered = await this.registerConnector(auth);
        if (!registered.success) return { persisted: true, registered, activated: null };
    ...
    private async persistInstallAuth(auth: ExtractedInstallAuth, rawPayload: Record<string, unknown>): Promise<StoredInstallAuth | null> {
    ...
    private async getLatestInstallAuth(): Promise<StoredInstallAuth | null> {
    ...
    private async updateInstallStatuses(
        installationId: string,
        input: {
            connectorId?: string | null;
            lineId?: string | null;
            connectorRegisteredAt?: Date | null;
            connectorRegistrationStatus?: string | null;
            connectorActivatedAt?: Date | null;
            connectorActivationStatus?: string | null;
            lastError?: string | null;
        },
    ) {
    ...
    private async callBitrixAppMethod(
        auth: StoredInstallAuth,
        method: string,
        payload: Record<string, unknown>,
    ): Promise<{ ok: boolean; data?: unknown; error?: string }> {
        const baseEndpoint = (auth.serverEndpoint || auth.clientEndpoint || '').replace(/\/+$/, '');
        if (!baseEndpoint) return { ok: false, error: 'server_endpoint/client_endpoint is missing in persisted install auth' };
        if (!auth.accessToken) return { ok: false, error: 'access token is missing in persisted install auth' };

        try {
            const response = await axios.post(`${baseEndpoint}/${method}.json`, { auth: auth.accessToken, ...payload }, { timeout: 15000 });
    ...
    private mapInstallRecord(record: {
        id: string;
        domain: string;
        memberId: string;
        accessToken: string;
    ...
    private extractInstallAuth(payload: Record<string, unknown>): ExtractedInstallAuth {
        const authNode = this.pickObject(payload, ['auth', 'AUTH']);
        const accessToken =
            this.pickStringFromObject(authNode, ['access_token', 'ACCESS_TOKEN']) ||
            this.pickString(payload, ['AUTH_ID', 'auth.access_token', 'AUTH.access_token', 'auth[access_token]', 'AUTH[access_token]', 'access_token']);
        const refreshToken =
            this.pickStringFromObject(authNode, ['refresh_token', 'REFRESH_TOKEN']) ||
        ...
        const status =
            this.pickStringFromObject(authNode, ['status', 'STATUS']) ||
            this.pickString(payload, ['status', 'AUTH_STATUS', 'auth.status', 'AUTH.status']);
        const clientEndpoint =
            this.pickStringFromObject(authNode, ['client_endpoint', 'CLIENT_ENDPOINT']) ||
        ...
        const serverEndpoint =
            this.pickStringFromObject(authNode, ['server_endpoint', 'SERVER_ENDPOINT']) ||
    ...
    private normalizePayload(value: Record<string, unknown>): Record<string, unknown> {
        ...
        for (const [rawKey, rawValue] of Object.entries(value || {})) {
            ...
            if (bracketPath.length > 1) this.assignPath(normalized, bracketPath, rawValue);
        }
        return normalized;
    }

    private deepCloneObject(value: Record<string, unknown>): Record<string, unknown> {
        const clone: Record<string, unknown> = {};
        for (const [key, entry] of Object.entries(value || {})) {
            clone[key] = entry && typeof entry === 'object' && !Array.isArray(entry)
                ? this.deepCloneObject(entry as Record<string, unknown>)
                : entry;
        }
        return clone;
    }

    private parseBracketKey(key: string): string[] {
    ...
    private pickObject(payload: Record<string, unknown>, keys: string[]): Record<string, unknown> | null {
    ...
    private pickString(payload: Record<string, unknown>, paths: string[]): string | null {
    ...
    private pickNumber(payload: Record<string, unknown>, paths: string[]): number | null {
        for (const path of paths) {
    ...
    private pickStringFromObject(payload: Record<string, unknown> | null, keys: string[]): string | null {
        return payload ? this.pickString(payload, keys) : null;
    }

    private pickNumberFromObject(payload: Record<string, unknown> | null, keys: string[]): number | null {
